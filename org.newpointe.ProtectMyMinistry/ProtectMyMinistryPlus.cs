...
namespace org.newpointe.ProtectMyMinistry
{
    ...
    public class ProtectMyMinistryPlus : BackgroundCheckComponent
    {
        ...
        public XDocument BuildRequest( string orderId, Person person, string ssn, DefinedValueCache requestType, string billingCode, List<String[]> counties = null )
        {
            ...
            if ( counties != null )
                locations.AddRange( counties.Select( c => new BGLocation { County = c[0], State = c[1] } ) );

            // Add their current or default location
            locations.Add( new BGLocation { County = county, State = state } );
        ...
        public BGLocation normalizeLocation( BGLocation loc )
        {
            if ( String.IsNullOrWhiteSpace( loc.State ) || String.IsNullOrWhiteSpace( loc.County ) )
                return null;

            loc.State = loc.State.Trim().ToLower();

            var stateId = DefinedTypeCache.Read( Rock.SystemGuid.DefinedType.LOCATION_ADDRESS_STATE.AsGuid() ).DefinedValues.Where( x => x.Description.ToLower() == loc.County ).FirstOrDefault()?.Value;
            if ( stateId != null )
                loc.State = stateId;
    ...
    public class BGLocationEquals : IEqualityComparer<BGLocation>
    {
        public bool Equals( BGLocation left, BGLocation right )
        {
            ...
            return ( left.State + ":" + left.County ) == ( right.State + ":" + right.County );
        }

        public int GetHashCode( BGLocation loc )
        {
            return ( loc.State + ":" + loc.County ).GetHashCode();
        }
    }
}
