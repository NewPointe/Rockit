...
namespace RockWeb.Plugins.org_newpointe.BlockMods.Core
{
    /// <summary>
    /// Block that can be used to set the default campus context for the site or page
    /// </summary>
    [DisplayName( "Custom Campus Context Setter" )]
    ...
    public partial class CampusContextSetter : RockBlock
    {
        ...
        protected void LoadDropdowns()
        {
            ...
            if (currentCampus == null && GetAttributeValue("DefaultToCurrentUser").AsBoolean() && CurrentPerson != null)
            {
                ...
                SetCampusContext(currentCampus != null ? currentCampus.Id : -1, false);
            }

            if (currentCampus == null)
            {
                currentCampus = new CampusService(new RockContext()).Get(GetAttributeValue("DefaultCampus").AsGuid());
                SetCampusContext(currentCampus != null ? currentCampus.Id : -1, false);
            }
            ...
            var campusList = CampusCache.All().Where(c => !c.Name.Contains("Central Services") && !c.Name.Contains("Online") && !c.Name.Contains("Future") && (c.IsActive ?? false))
