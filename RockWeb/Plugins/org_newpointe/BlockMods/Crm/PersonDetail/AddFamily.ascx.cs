...
namespace RockWeb.Plugins.org_newpointe.BlockMods.Crm.PersonDetail
{
    ...
    [GroupRoleField( Rock.SystemGuid.GroupType.GROUPTYPE_FAMILY, "Default Group Role",
        "The group role that should be set by default", false, Rock.SystemGuid.GroupRole.GROUPROLE_FAMILY_MEMBER_ADULT, "", 2 )]
    [BooleanField( "Gender", "Require a gender for each person", "Don't require", "Should Gender be required for each person added?", false, "", 3 )]
    [BooleanField( "Address", "Ask for an address", "Don't ask", "Should user be asked for an adress for the family?", true, "", 4 )]
    [DefinedValueField( Rock.SystemGuid.DefinedType.PERSON_MARITAL_STATUS, "Adult Marital Status", "The default marital status for adults in the family.", false, false, "", "", 5 )]
    ...
    [BooleanField( "Marital Status Confirmation", "Should user be asked to confirm saving an adults without a marital status?", true, "", 7 )]
    [BooleanField( "Grade", "Require a grade for each child", "Don't require", "Should Grade be required for each child added?", false, "", 8 )]
    ...
    [BooleanField( "ShowRole", "Show Role", "Hide Role", "Should the role be shown?", true, "", 10 )]
    [BooleanField( "ShowTitle", "Show Title", "Hide Title", "Should the title be shown?", true, "", 11 )]
    [BooleanField( "ShowSuffix", "Show Suffix", "Hide Suffix", "Should the suffix be shown?", true, "", 12 )]
    [BooleanField( "ShowConnectionStatus", "Show ConnectionStatus", "Hide ConnectionStatus", "Should the ConnectionStatus be shown?", true, "", 13 )]
    ...
    public partial class AddFamily : Rock.Web.UI.RockBlock
    {
        ...
        protected override void OnInit( EventArgs e )
        {
            ...
            acAddress.Visible = GetAttributeValue( "Address" ).AsBoolean();

            if ( !GetAttributeValue( "ShowRole" ).AsBoolean() )
            {
                nfmMemberDiv.AddCssClass( "hide-role" );
            }
            if ( !GetAttributeValue( "ShowTitle" ).AsBoolean() )
            {
                nfmMemberDiv.AddCssClass( "hide-title" );
            }
            if ( !GetAttributeValue( "ShowSuffix" ).AsBoolean() )
            {
                nfmMemberDiv.AddCssClass( "hide-suffix" );
            }
            if ( !GetAttributeValue( "ShowConnectionStatus" ).AsBoolean() )
            {
...
", ddlMaritalStatus.ClientID );
        ...
        private void AddFamilyMember()
        {
            ...
            if ( familyGroupType != null && familyGroupType.DefaultGroupRoleId != null )
            {
                ...
                var GroupRole = GroupTypeCache.Read( Rock.SystemGuid.GroupType.GROUPTYPE_FAMILY ).Roles.Where( x => x.Guid == GetAttributeValue( "DefaultGroupRole" ).AsGuid() ).FirstOrDefault();

                dbgTxt.Text += GetAttributeValue( "DefaultGroupRole" ).AsGuid() + "<br/>";
                ...
                familyMember.GroupRoleId = (GroupRole != null) ? GroupRole.Id : familyGroupType.DefaultGroupRoleId.Value;
